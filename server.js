...
app.get('/equipment/:employeeID', function (req, res, next) {
  pool.query('SELECT equipment_type.name, equipment.calibration_date, equipment.purchase_date FROM `equipment` INNER JOIN `equipment_type` ON equipment.type_id = equipment_type.id WHERE equipment.maintainer_id = ?',[req.params.id]).then(function (results, fields) {
    res.status(200);
    res.json(results);
  }).catch(function (err) {
      console.error(err.stack);
